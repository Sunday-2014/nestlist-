...
export default function ListingDetail({ params }) {
  ...
  const isJob = listing?.listing_category === 'Job'
  const isVehicle = listing?.listing_category === 'Vehicle'
  const isProperty = !isJob && !isVehicle
  ...
  const Footer = () => (
    <footer style={{background:'#1f2937', borderTop:'3px solid #ea580c', padding:'24px 16px', textAlign:'center'}}>
      ...
      <p style={{fontSize:'12px', color:'#9ca3af', margin:'0'}}>Free listings · No fees · Connect directly with owners</p>
    </footer>
  )
  ...
  const shareText = `Check out this listing: ${listing.title} on EnjeraPressList.Com`
    ...
    <div style={{minHeight:'100vh', background:'#f8fafc', fontFamily:'system-ui, -apple-system, sans-serif', overflowX:'hidden'}}>
      ...
      <div style={{maxWidth:'900px', margin:'0 auto', padding:'24px 16px 64px', boxSizing:'border-box'}}>
        ...
        {images.length > 0 ? (
          ...
          <div style={{height:'200px', background: isJob ? 'linear-gradient(135deg, #eff6ff, #dbeafe)' : isVehicle ? 'linear-gradient(135deg, #f5f3ff, #ede9fe)' : 'linear-gradient(135deg, #fff7ed, #ffedd5)', borderRadius:'16px', display:'flex', alignItems:'center', justifyContent:'center', fontSize:'64px', marginBottom:'16px', border:'1px solid #e5e7eb'}}>
            {isJob ? '💼' : isVehicle ? '🚗' : '🏠'}
          </div>
        )}
        ...
        <div style={{background:'#ffffff', borderRadius:'16px', padding:'16px', border:'1px solid #e5e7eb', marginBottom:'16px', boxShadow:'0 1px 4px rgba(0,0,0,0.05)'}}>
          ...
          <div style={{display:'grid', gridTemplateColumns:'1fr 1fr', gap:'8px', marginBottom:'8px'}}>
            ...
            <a href={`https://twitter.com/intent/tweet?text=${encodeURIComponent(shareText)}&url=${encodeURIComponent(pageUrl)}`} target="_blank" rel="noopener noreferrer"
              style={{padding:'11px 6px', borderRadius:'8px', background:'#000000', color:'#ffffff', fontSize:'13px', fontWeight:'700', textAlign:'center', textDecoration:'none', display:'block'}}>𝕏 Twitter</a>
          </div>
          <button onClick={handleCopyLink}
            style={{width:'100%', padding:'10px', borderRadius:'8px', background: copied ? '#f0fdf4' : '#f9fafb', color: copied ? '#166534' : '#374151', fontSize:'13px', fontWeight:'700', border: copied ? '2px solid #bbf7d0' : '2px solid #e5e7eb', cursor:'pointer', boxSizing:'border-box'}}>
        ...
        <div style={{background:'#ffffff', borderRadius:'16px', padding:'20px', border:'1px solid #e5e7eb', boxShadow:'0 4px 16px rgba(0,0,0,0.08)', marginBottom:'16px'}}>
          <h2 style={{fontSize:'16px', fontWeight:'700', color:'#111827', margin:'0 0 16px', paddingBottom:'10px', borderBottom:'2px solid #fed7aa'}}>
            {isJob ? '📞 Contact / አድራሻ' : isVehicle ? '📞 Contact Seller' : '📞 Contact Landlord'}
          </h2>
        ...
        <div style={{background:'#ffffff', borderRadius:'16px', padding:'20px', border:'1px solid #e5e7eb', marginBottom:'16px', boxShadow:'0 1px 4px rgba(0,0,0,0.05)'}}>
          <div style={{display:'flex', alignItems:'center', gap:'8px', marginBottom:'10px', flexWrap:'wrap'}}>
            {isJob && (
              <>
                <span style={{background: listing.job_employment_type === 'Hiring' ? '#166534' : '#1877F2', color:'#ffffff', fontSize:'11px', fontWeight:'700', padding:'4px 10px', borderRadius:'6px'}}>
                  {listing.job_employment_type === 'Hiring' ? '🏢 Hiring — ሰራተኛ ያስፈልጋል' : '👤 Looking for Work — ስራ እፈልጋለሁ'}
                </span>
                {listing.job_type && <span style={{background:'#f9fafb', color:'#374151', fontSize:'11px', fontWeight:'700', padding:'4px 10px', borderRadius:'6px', border:'1px solid #e5e7eb'}}>{listing.job_type}</span>}
              </>
            )}
            {isVehicle && (
              <>
            ...
            <div style={{display:'flex', flexWrap:'wrap', gap:'10px'}}>
              {listing.job_salary && (
                <div style={{background:'#f0fdf4', borderRadius:'10px', padding:'10px 16px', border:'1px solid #bbf7d0', textAlign:'center'}}>
                  <p style={{fontSize:'11px', color:'#166534', margin:'0 0 2px', fontWeight:'700', textTransform:'uppercase', letterSpacing:'0.05em'}}>Salary / ደሞዝ</p>
                  <p style={{fontSize:'18px', fontWeight:'800', color:'#166534', margin:'0'}}>{listing.job_salary}</p>
                </div>
              )}
              {listing.job_type && (
                <div style={{background:'#eff6ff', borderRadius:'10px', padding:'10px 16px', border:'1px solid #bfdbfe', textAlign:'center'}}>
                  <p style={{fontSize:'11px', color:'#1877F2', margin:'0 0 2px', fontWeight:'700', textTransform:'uppercase', letterSpacing:'0.05em'}}>Job Type</p>
                  <p style={{fontSize:'16px', fontWeight:'800', color:'#1877F2', margin:'0'}}>{listing.job_type}</p>
                </div>
              )}
              {listing.job_experience && (
                <div style={{background:'#fdf4ff', borderRadius:'10px', padding:'10px 16px', border:'1px solid #e9d5ff', textAlign:'center'}}>
                  <p style={{fontSize:'11px', color:'#7c3aed', margin:'0 0 2px', fontWeight:'700', textTransform:'uppercase', letterSpacing:'0.05em'}}>Experience</p>
                  <p style={{fontSize:'16px', fontWeight:'800', color:'#7c3aed', margin:'0'}}>{listing.job_experience}</p>
                </div>
              )}
              {listing.job_company && (
                <div style={{background:'#f9fafb', borderRadius:'10px', padding:'10px 16px', border:'1px solid #e5e7eb', textAlign:'center'}}>
                  <p style={{fontSize:'11px', color:'#6b7280', margin:'0 0 2px', fontWeight:'700', textTransform:'uppercase', letterSpacing:'0.05em'}}>Company</p>
                  <p style={{fontSize:'16px', fontWeight:'800', color:'#111827', margin:'0'}}>{listing.job_company}</p>
                </div>
              )}
            </div>
          )}
          ...
          {isVehicle && (
            <div style={{display:'flex', flexWrap:'wrap', gap:'10px'}}>
              {(listing.sale_price > 0 || listing.price > 0) && listing.currency !== 'Contact' && (
                <div style={{background:'#f5f3ff', borderRadius:'10px', padding:'10px 16px', border:'1px solid #ddd6fe', textAlign:'center'}}>
                  <p style={{fontSize:'11px', color:'#7c3aed', margin:'0 0 2px', fontWeight:'700', textTransform:'uppercase', letterSpacing:'0.05em'}}>{listing.listing_type === 'Rent' ? 'Rental Price' : 'Sale Price'}</p>
                  <p style={{fontSize:'20px', fontWeight:'800', color:'#7c3aed', margin:'0'}}>
                    {listing.currency === 'ETB'
                      ? `${(listing.listing_type === 'Rent' ? listing.price : listing.sale_price)?.toLocaleString()} ETB`
                      : `$${(listing.listing_type === 'Rent' ? listing.price : listing.sale_price)?.toLocaleString()}`
                    }
                  </p>
                  {listing.listing_type === 'Rent' && listing.price_period && <p style={{fontSize:'11px', color:'#7c3aed', margin:'2px 0 0'}}>{listing.price_period}</p>}
                </div>
              )}
              {listing.vehicle_year && (
                <div style={{background:'#f9fafb', borderRadius:'10px', padding:'10px 16px', border:'1px solid #e5e7eb', textAlign:'center'}}>
                  <p style={{fontSize:'11px', color:'#6b7280', margin:'0 0 2px', fontWeight:'700', textTransform:'uppercase', letterSpacing:'0.05em'}}>Year</p>
                  <p style={{fontSize:'16px', fontWeight:'800', color:'#111827', margin:'0'}}>{listing.vehicle_year}</p>
                </div>
              )}
              {listing.vehicle_make && (
                <div style={{background:'#f9fafb', borderRadius:'10px', padding:'10px 16px', border:'1px solid #e5e7eb', textAlign:'center'}}>
                  <p style={{fontSize:'11px', color:'#6b7280', margin:'0 0 2px', fontWeight:'700', textTransform:'uppercase', letterSpacing:'0.05em'}}>Make</p>
                  <p style={{fontSize:'16px', fontWeight:'800', color:'#111827', margin:'0'}}>{listing.vehicle_make}</p>
                </div>
              )}
              {listing.vehicle_model && (
                <div style={{background:'#f9fafb', borderRadius:'10px', padding:'10px 16px', border:'1px solid #e5e7eb', textAlign:'center'}}>
                  <p style={{fontSize:'11px', color:'#6b7280', margin:'0 0 2px', fontWeight:'700', textTransform:'uppercase', letterSpacing:'0.05em'}}>Model</p>
                  <p style={{fontSize:'16px', fontWeight:'800', color:'#111827', margin:'0'}}>{listing.vehicle_model}</p>
                </div>
              )}
              {listing.vehicle_mileage && (
                <div style={{background:'#f9fafb', borderRadius:'10px', padding:'10px 16px', border:'1px solid #e5e7eb', textAlign:'center'}}>
                  <p style={{fontSize:'11px', color:'#6b7280', margin:'0 0 2px', fontWeight:'700', textTransform:'uppercase', letterSpacing:'0.05em'}}>Mileage</p>
                  <p style={{fontSize:'16px', fontWeight:'800', color:'#111827', margin:'0'}}>{listing.vehicle_mileage}</p>
                </div>
              )}
              {listing.vehicle_condition && (
                <div style={{background:'#f0fdf4', borderRadius:'10px', padding:'10px 16px', border:'1px solid #bbf7d0', textAlign:'center'}}>
                  <p style={{fontSize:'11px', color:'#166534', margin:'0 0 2px', fontWeight:'700', textTransform:'uppercase', letterSpacing:'0.05em'}}>Condition</p>
                  <p style={{fontSize:'16px', fontWeight:'800', color:'#166534', margin:'0'}}>{listing.vehicle_condition}</p>
                </div>
              )}
            </div>
          )}
          ...
          {isProperty && (
            <div style={{display:'flex', flexWrap:'wrap', gap:'10px'}}>
              ...
              {isForSale && listing.currency !== 'Contact' && listing.sale_price > 0 && (
                <div style={{background:'#f0fdf4', borderRadius:'10px', padding:'10px 16px', border:'1px solid #bbf7d0', textAlign:'center'}}>
              ...
              {isForSale && listing.down_payment > 0 && (
                <div style={{background:'#eff6ff', borderRadius:'10px', padding:'10px 16px', border:'1px solid #bfdbfe', textAlign:'center'}}>
              ...
              {isForSale && listing.monthly_after_down > 0 && (
                <div style={{background:'#fdf4ff', borderRadius:'10px', padding:'10px 16px', border:'1px solid #e9d5ff', textAlign:'center'}}>
        ...
        {listing.description && (
          <div style={{background:'#ffffff', borderRadius:'16px', padding:'20px', border:'1px solid #e5e7eb', marginBottom:'16px', boxShadow:'0 1px 4px rgba(0,0,0,0.05)'}}>
            <h2 style={{fontSize:'16px', fontWeight:'700', color:'#111827', margin:'0 0 12px', paddingBottom:'10px', borderBottom:'2px solid #fed7aa'}}>
              {isJob ? '💼 Job Description / የስራ ዝርዝር' : isVehicle ? '🚗 Vehicle Description' : '🏠 About this Property'}
            </h2>
        ...
        {videos.length > 0 && (
          <div style={{background:'#ffffff', borderRadius:'16px', padding:'20px', border:'1px solid #e5e7eb', marginBottom:'16px', boxShadow:'0 1px 4px rgba(0,0,0,0.05)'}}>
            <h2 style={{fontSize:'16px', fontWeight:'700', color:'#111827', margin:'0 0 12px', paddingBottom:'10px', borderBottom:'2px solid #fed7aa'}}>🎥 Video</h2>
